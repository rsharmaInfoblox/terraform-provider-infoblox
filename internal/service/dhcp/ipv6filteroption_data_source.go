...
var _ datasource.DataSourceWithValidateConfig = &Ipv6filteroptionDataSource{}
var _ datasource.DataSourceWithConfigure = &Ipv6filteroptionDataSource{}
...
func (d *Ipv6filteroptionDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_ipv6_filteroption"
}

// Ipv6filteroptionDataSourceModel is the filter model for the datasource
type Ipv6filteroptionDataSourceModel struct {
...
func (m *Ipv6filteroptionDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.Ipv6filteroption, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: Ipv6filteroptionAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &Ipv6filteroptionModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, Ipv6filteroptionAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: Ipv6filteroptionAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *Ipv6filteroptionDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox Ipv6filteroption from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(Ipv6filteroptionResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *Ipv6filteroptionDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewIpv6filteroptionService(d.backend, client.NIOS, client.UDDI)
}

func (d *Ipv6filteroptionDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data Ipv6filteroptionDataSourceModel
...
func (d *Ipv6filteroptionDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.Ipv6filteroption
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.Ipv6filteroption, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.Ipv6filteroption, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list Ipv6filteroption records: %s", err))
		return
	}
